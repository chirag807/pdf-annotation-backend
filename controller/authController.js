...
export const createUserController = async (request, response) => {
  try {
    ...
    if (userData.status !== 201) {
      return response.status(userData.status).json({ message: userData.message });
    }

    response.status(201).json({
      message: "User registered successfully",
      token: userData.token,
      user: { id: userData.user._id, email: userData.user.email, name: userData.user.name, role: userData.user.role },
    });
  } catch (error) {
